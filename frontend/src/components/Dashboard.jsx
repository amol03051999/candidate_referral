...
const Dashboard = () => {
  ...
  const updateStatus = async (id, status) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/candidates/${id}/status`, { status });
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-lg">
      ...
      <div className="grid gap-4">
        {filteredCandidates.map(candidate => (
          <div key={candidate._id} className="border rounded-lg p-6 hover:shadow-md">
            ...
            <p className="text-gray-600 mb-1"><strong>Job:</strong> {candidate.jobTitle}</p>
            <p className="text-gray-600 mb-1"><strong>Email:</strong> {candidate.email}</p>
            <p className="text-gray-600 mb-4"><strong>Phone:</strong> {candidate.phone}</p>
            {candidate.resumeUrl && (
              <a href={`http://localhost:5000${candidate.resumeUrl}`} 
                 target="_blank" rel="noopener noreferrer" 
                 className="text-blue-600 hover:underline text-sm">View Resume</a>
            )}
            <div className="flex gap-2 mt-4">
              <select onChange={(e) => updateStatus(candidate._id, e.target.value)}
      ...
      {filteredCandidates.length === 0 && (
        <p className="text-center text-gray-500 py-12">{search ? 'No matching candidates' : 'No candidates yet'}</p>
      )}
    </div>
  );
};
